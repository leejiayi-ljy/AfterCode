    ...
    >
      {code_review.strengths.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <span className='label' style={{ color: 'var(--green)', fontSize: '8px' }}>
            strengths
          </span>
      ...
      {code_review.improvements.length > 0 && (
        <div
          style={{
            ...
            gap: '6px',
            marginTop: code_review.strengths.length ? '6px' : 0,
          }}
        >
          <span className='label' style={{ color: 'var(--orange)', fontSize: '8px' }}>
            improvements
          </span>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {code_review.improvements.map((item, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <p style={{ fontSize: '12.5px', color: 'var(--text)', lineHeight: '1.55' }}>
                  {item.issue}
                </p>
                <p
                  style={{
